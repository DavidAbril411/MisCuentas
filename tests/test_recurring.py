...
from miscuentas.models import Account, RecurringRule, Transaction, FxRate
...
def test_materialize_idempotent(session):
    a = Account(name="Pesos", currency="ARS", opening_balance_minor=0)
    session.add(a)
    session.flush()
    rule = RecurringRule(
        name="Mensual", kind="expense",
    ...
    session.add(rule)
    session.flush()
    n1 = materialize_recurring(session, date(2026, 5, 13))
    session.commit()
    n2 = materialize_recurring(session, date(2026, 5, 13))
    session.commit()
    assert n1 == 5  # Jan..May
    assert n2 == 0
...
def test_materialize_usd_uses_rate_on_occurrence(session):
    a = Account(name="USD", currency="USD", opening_balance_minor=0)
    session.add(a)
    session.flush()
    set_rate(session, date(2026, 1, 1), "1300")
    set_rate(session, date(2026, 5, 1), "1400")
    session.commit()
    rule = RecurringRule(
        name="USD sub", kind="expense",
    ...
    session.add(rule)
    session.flush()
    materialize_recurring(session, date(2026, 5, 13))
    session.commit()
    txs = (session.query(Transaction)
           ...
           .all())
    assert len(txs) == 5
    assert txs[0].occurred_on == date(2026, 1, 1)
    assert txs[0].fx_rate_to_ars_micro == fx_to_micro("1300")
    assert txs[4].occurred_on == date(2026, 5, 1)
